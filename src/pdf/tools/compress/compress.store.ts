...
import { CompressLevel, getCompressQuality } from "./index";
...
export const useCompressState = create<CompressState>((set, get) => ({
  ...
  runCompress: async () => {
    ...
    if (file && destinationPath && !isLoading) {
      ...
      if (result.ok) {
        notifySuccess(
          i18next.t("tools:tools.compress.notifications.success") ||
        ...
        notifyError(
          result.error ??
            (i18next.t("tools:tools.compress.notifications.error") ||
      ...
      notifyWarning(
        i18next.t("tools:tools.compress.notifications.warning") ||
          "Please select a file and a destination",
      );
    }
  },
}));
